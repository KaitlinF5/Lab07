...
*The distribution of the variable score is skewed to the left. Suggesting there are more students who are satisfied with the proffesors compared to the students who aren't.*
...
*Jitter adds small variation to the points to help with the overlapping of points. The previous graph had points that overlapped completely, so not all points were visible clearly, which can be misleading.*
...
*score-hat = 3.88 + 0.067 x bty_avg*

2. Plot the data again using `geom_jitter()`, and add the regression line.

```{r viz-score_bty_fit,eval=FALSE}
score_bty_fit_aug <- augment(score_bty_fit$fit)
ggplot(data = score_bty_fit_aug) + 
